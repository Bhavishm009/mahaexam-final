...
export default function NotificationCenter() {
  ...
  async function handleMarkAllRead() {
    try {
      ...
      toast.success(
        language === "mr"
          ? "सर्व सूचना वाचून झाल्या म्हणून चिन्हांकित केल्या"
  ...
  return (
    <div className="relative font-sans">
      <button
        type="button"
        onClick={() => setOpen((x) => !x)}
        className="shadow-xs relative flex items-center gap-1.5 rounded-2xl border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-bold text-slate-700 transition hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800 sm:px-3 sm:py-2"
        title={isMr ? "सूचना केंद्र" : "Notification Center"}
      >
        <Bell className="h-4 w-4 text-slate-600 dark:text-slate-400" />
        {/* <span className="hidden sm:inline">{isMr ? "सूचना" : "Alerts"}</span> */}
        {unread > 0 && (
          <span className="shadow-xs flex h-4 min-w-4 animate-pulse items-center justify-center rounded-full bg-rose-600 px-1 text-[10px] font-black text-white">
            {unread}
          </span>
        )}
      </button>
      ...
      {open && (
        <>
          <div
            className="backdrop-blur-xs fixed inset-0 z-40 bg-slate-900/10 sm:bg-transparent"
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-semibold text-slate-400">
                    {unread > 0
                      ? `${unread} ${isMr ? "नवीन संदेश" : "unread alert(s)"}`
                      : isMr
                        ? "सर्व वाचून झाले"
                        : "All caught up"}
                  </p>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            <h4 className="truncate text-xs font-extrabold text-slate-900 group-hover:text-blue-600 dark:text-white dark:group-hover:text-blue-400">
                              {n.title}
                            </h4>
                            {targetUrl && (
                          ...
                          <p className="mt-1 line-clamp-2 text-xs font-medium text-slate-600 dark:text-slate-300">
                            {n.message}
                          </p>
