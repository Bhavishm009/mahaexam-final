  ...
  return (
    <main className="min-h-screen bg-slate-50 p-6 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="mx-auto max-w-7xl">
        <h1 className="text-3xl font-black text-slate-900 dark:text-white">Financial Control Center</h1>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {[
            ...
            <div key={label} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="text-sm font-medium text-slate-500 dark:text-slate-400">{label}</div>
              <div className="mt-2 text-3xl font-black text-slate-900 dark:text-white">₹{Number(val).toFixed(2)}</div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <h2 className="text-xl font-black text-slate-900 dark:text-white">Marketplace Transfers</h2>
          ...
          <div className="mt-4 overflow-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {d.transfers?.map((x) => (
                  <tr key={x.id} className="border-b border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200">
                    <td className="p-3 font-medium text-slate-900 dark:text-white">{x.organization?.name}</td>
