...
export default function EditBlogPage({ params }) {
  ...
  useEffect(() => {
    async function fetchBlog() {
      ...
      try {
        setLoading(true);
        const res = await fetch(`/api/admin/blogs?id=${encodeURIComponent(blogId)}`);
        ...
        setFormData({
          ...
          slug: b.slug || "",
          content: b.content || "",
          contentMr: b.contentMr || "",
          excerpt: b.excerpt || "",
          imageUrl: b.imageUrl || "",
          category: b.category || "Police Bharti",
          published: b.published ?? true,
          authorName: b.authorName || "MahaExam Editorial Team",
        });
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex h-96 flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm font-medium text-slate-500">Loading blog post details...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="font-mono text-xs text-slate-500 dark:text-slate-400">
            ID: {blogId} {formData.slug ? `· /blogs/${formData.slug}` : ""}
          </p>
        </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Use the rich formatting toolbar to edit headings, lists, links, quotes, and styling.
              </p>
            </div>
            ...
            <RichTextEditor
              ...
              minHeight="300px"
              helperText="You can switch to HTML source mode to inspect or fine-tune formatting."
            />

            {/* Marathi Content */}
            <RichTextEditor
              label="Article Content (मराठी)"
