        ...
        <div className="mt-8 overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-800 dark:text-slate-200">
              {items.map((p) => (
                <tr key={p.id} className="border-t border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/40">
                  <td className="p-4 text-slate-600 dark:text-slate-400">{new Date(p.createdAt).toLocaleString()}</td>
                  ...
                  <td className="text-slate-700 dark:text-slate-300">{p.organization?.name || p.user?.email || "—"}</td>
                  <td className="font-mono text-xs text-slate-500 dark:text-slate-400">{p.razorpayPaymentId || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
