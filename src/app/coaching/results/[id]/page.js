  ...
  return (
    <main className="min-h-screen bg-slate-50 p-6 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="mx-auto max-w-7xl">
        <h1 className="text-3xl font-black text-slate-900 dark:text-white">{data.exam?.title || "Exam"} — Results</h1>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-800 dark:text-slate-200">
              {data.results?.map((r) => (
                <tr key={r.id} className="border-t border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/40">
                  ...
                  <td>
                    <div className="font-semibold text-slate-900 dark:text-white">{r.student?.name || "Student"}</div>
                  ...
                  <td className="text-emerald-600 dark:text-emerald-400 font-medium">{r.correct}</td>
                  <td className="text-rose-600 dark:text-rose-400 font-medium">{r.wrong}</td>
                  <td className="text-slate-500 dark:text-slate-400">{r.unanswered}</td>
                  <td className={r.passed ? "font-bold text-emerald-600 dark:text-emerald-400" : "font-bold text-rose-600 dark:text-rose-400"}>
                    {r.passed ? "PASS" : "FAIL"}
                  </td>
                </tr>
              ))}
